...
export default function SortamentNav({ profiles, selected, highlighted = [], onSelect, mobileOpen, onMobileClose }: Props) {
  const renderList = (fontSize = 13, padding = '8px 12px') => profiles.map(p => {
    ...
    return (
      <button key={p.key} onClick={() => { onSelect(p.key); onMobileClose?.() }} style={{
      ...
      >
        <img src={`/icons/${p.icon}.svg`} alt="" width={28} height={28}
  ...
  if (mobileOpen === undefined) return (
    <div style={{
      ...
      <div style={{ fontSize: 10, fontWeight: 700, letterSpacing: '.08em', color: 'var(--on-surface-variant)', padding: '12px 14px 6px', textTransform: 'uppercase' }}>
        Сортамент
      </div>
      {renderList()}
    </div>
